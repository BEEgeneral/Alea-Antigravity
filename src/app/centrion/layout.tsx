...
const navItems: NavItem[] = [
    ...
    return (
        <div className="min-h-screen bg-background flex">
            {/* Sidebar */}
            <aside className="w-64 border-r border-border/50 bg-card/50 backdrop-blur-xl flex flex-col fixed h-full overflow-y-auto">
                {/* Header */}
                ...
                <nav className="flex-1 p-4 space-y-1">
                    {navItems.map((item) => {
                        const isActive = pathname === item.href || (item.href !== "/centrion" && pathname.startsWith(item.href));
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-primary/10 text-primary' : 'hover:bg-muted'}`}
                            >
                ...
                <div className="p-4 border-t border-border/50">
                    <div className="p-4 bg-muted/50 rounded-xl">
                        ...
                        <p className="text-xs font-medium text-green-500">Todos los servicios activos</p>
                    </div>
                </div>
            </aside>
